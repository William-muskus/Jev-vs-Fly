...
class TrainConfig:
    ...
    # ---- self-play (v2: human-mixed training, gated promotion) ----
    selfplay_iters: int = 50
    selfplay_games_per_iter: int = 512          # games per iteration, always generated by the best network
    mcts_sims: int = 100
    c_puct: float = 1.5
    ...
    selfplay_batch_size: int = 384
    selfplay_train_steps_per_iter: int = 400
    selfplay_lr: float = 1e-4
    max_game_plies: int = 300
    selfplay_workers: int = 1
    selfplay_lockstep_games: int = 512          # most games searched in one lockstep batch (more -> several batches)
    selfplay_eval_every_iters: int = 5          # Elo vs random (and previous iteration without a gate) every N iterations
    selfplay_human_mix: float = 0.5             # fraction of every training batch streamed from the imitation shards
    selfplay_value_blend: float = 0.5           # value target = blend * outcome + (1 - blend) * root search value
    selfplay_gate: bool = True                  # promote the candidate only if it beats the best network
    selfplay_gate_games: int = 40               # gate match: candidate vs best, superfly, paired openings, both colours
    selfplay_gate_sims: int = 100               # MCTS simulations per move in the gate match
    selfplay_gate_threshold: float = 0.55       # promote iff the candidate's score fraction >= threshold
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("log_every", "eval_every", "checkpoint_every", "eval_batches", "elo_games", "num_workers",
                     "warmup_steps", "elo_every", "selfplay_eval_every_iters", "selfplay_games_per_iter", "mcts_sims",
                     "selfplay_batch_size", "selfplay_lockstep_games", "selfplay_gate_games", "selfplay_gate_sims"):
            v = getattr(self, name)
            floor = 0 if name in ("num_workers", "warmup_steps", "elo_every", "selfplay_gate_games") else 1
        ...
        for name in ("selfplay_human_mix", "selfplay_value_blend", "selfplay_gate_threshold"):
            v = getattr(self, name)
        ...
        if not isinstance(self.selfplay_gate, bool):
            raise TypeError(f"selfplay_gate must be a bool, got {self.selfplay_gate!r}")
    ...
    def tiny(cls, run: str = "tiny", **overrides: Any) -> TrainConfig:
        ...
        cfg = cls(
            ...
            selfplay_gate_games=2,
            selfplay_gate_sims=4,
        )
