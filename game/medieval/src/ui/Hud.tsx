...
function sideName(snapshot: GameSnapshot, faction: Faction): string {
  return snapshot.sideNames?.[faction] ?? (faction === "w" ? "Ivory" : "Obsidian");
};
            ...
            <div>
              ...
              <p className="mc-display text-sm text-[#f2e2bd]">
                {snapshot.status === "over"
                  ...
                  : snapshot.thinking
                    ? "Thinking…"
                    : sideName(snapshot, snapshot.turn)}
              </p>
            </div>
...
function TallyRow({
  ...
  return (
    <div className="mc-tally-row" data-faction={faction} data-live={onMove || undefined}>
      ...
      <span className="mc-tally-name">{name}</span>
