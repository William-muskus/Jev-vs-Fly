...
import { heraldCall } from "../core/wizardChess";
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        {phase === "playing" && snapshot.moves.length > 0 ? (
          <div className="mc-herald pointer-events-none" key={snapshot.moves.length}>
            {heraldCall(snapshot.moves[snapshot.moves.length - 1])}
          </div>
        ) : null}
...
function LoadingScreen({ progress }: { progress: number }) {
  return (
    <div className="mc-fade absolute inset-0 flex flex-col items-center justify-center gap-5 bg-[#05060a]/85 px-6">
      <p className="mc-display text-[0.62rem] tracking-[0.5em] text-[#a89268]">THE PIECES ARE ENCHANTED</p>
      <h1 className="mc-display mc-title-glow text-4xl text-[#f4e3bd]">WIZARD CHESS</h1>
      <p className="mc-display text-[0.7rem] tracking-[0.35em] text-[#c8ab74]">JEV vs FLY</p>
      ...
      <p className="text-xs italic text-[#7d6f57]">Awakening {Math.round(progress * 6)} of 6 pieces…</p>
    </div>
  );
}
