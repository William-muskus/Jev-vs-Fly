 ...
 * {@link ARM_SCULPTS}).
 */
export type ArsenalId = "kingdom" | "sun" | "empire" | "none";

/** Selectable army skins. Either side of the board can wear any of them. */
export type ArmySkinId = "ivory" | "sun" | "empire" | "wizard";

/** Static (unrigged) sculpt per kind — the fallback when a rig fails to load. */
const STILL_MODELS: Record<ArmySkinId, Roster<string>> = {
  // Local GLBs if `python -m game.wizard_assets` has been run; otherwise the
  // factory builds gothic-Staunton stone in `buildWizardChessPiece`.
  wizard: {
    k: "procedural:wizard",
    q: "procedural:wizard",
    b: "procedural:wizard",
...
export const ARMY_SKINS: Record<ArmySkinId, ArmySkin> = {
  wizard: {
    ...
    arsenal: "none",
    native: "w",
    still: STILL_MODELS.wizard,
    animated: ANIMATED_MODELS.wizard,
    cries: DEATH_CRIES.wizard,
  },
  ivory: {
    id: "ivory",
    label: "Ivory Kingdom",
...
export const ARMY_SKIN_ORDER: ArmySkinId[] = ["wizard", "ivory", "sun", "empire"];

/** Which army each side musters until the player says otherwise. */
export const DEFAULT_ARMY_SKINS: Record<Faction, ArmySkinId> = { w: "wizard", b: "wizard" };
...
export const ARMY_CARDS: Record<ArmySkinId, string> = {
  wizard: "https://r2-pub.rork.com/projects/g9111r67kl6tq85g540sd/assets/dc88fa56-b565-4cf8-aa98-9997c8685e5d.png",
