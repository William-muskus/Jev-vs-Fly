...
export class GameController extends Emitter<ControllerEvents> {
  ...
  setMovers(
    movers: Partial<Record<Faction, (fen: string, historyUci: string[]) => Promise<EngineMove | null>>>,
    names?: { w: string; b: string },
  ): void {
    this.movers = movers;
    if (names) this.sideNames = names;
  }

  clearMovers(): void {
    this.movers = {};
    this.sideNames = { w: "Ivory", b: "Obsidian" };
  }

  setMaxPlies(n: number | null): void {
    this.maxPlies = n;
  }
  ...
  private async maybeRunEngine(): Promise<void> {
    ...
    const mover = this.movers[turn];
    const historyUci = (this.chess.history({ verbose: true }) as Move[]).map(
      (m) => m.from + m.to + (m.promotion ?? ""),
    );
    let best: EngineMove | null;
    try {
      best = mover ? await mover(this.chess.fen(), historyUci) : await this.ai.bestMove(this.chess.fen(), difficulty);
    } catch (err) {
      console.error("[engine] mover failed", err);
      best = null;
    }
  ...
  private buildSnapshot(): GameSnapshot {
    ...
    return {
      ...
      sideNames: { ...(this.sideNames ?? { w: "Ivory", b: "Obsidian" }) },
    };
  }
